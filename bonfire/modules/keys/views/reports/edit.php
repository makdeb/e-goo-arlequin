...
<div>
        <?php echo form_label('Ключ доступа', 'key'); ?>
...
<div>
        ...
        <input id="valid_untill" type="text" name="valid_untill" maxlength="20" value="<?php echo set_value('valid_untill', isset($keys['valid_untill']) ? date("d.m.Y H:i:s",strtotime($keys['valid_untill'])) : ''); ?>"  />
</div>
...
<div>
        ...
        <input id="ordered_on" type="text" name="ordered_on" maxlength="20" value="<?php echo set_value('ordered_on', isset($keys['ordered_on']) ? date("d.m.Y H:i:s",strtotime($keys['ordered_on'])) : ''); ?>"  />
</div>
...
<div>
        ...
        <input id="bought_on" type="text" name="bought_on" maxlength="20" value="<?php echo set_value('bought_on', isset($keys['bought_on']) ? date("d.m.Y H:i:s",strtotime($keys['bought_on'])) : ''); ?>"  />
</div>
...
<div>
	<?php echo form_label('Оплачен', 'is_paid'); ?>               
        <input type="checkbox" id="is_paid" name="is_paid" value="1" <?php echo (isset($keys['is_paid']) && $keys['is_paid'] == 1) ? 'checked="checked"' : set_checkbox('is_paid', 1); ?>> 	
</div> 
	...
	<div class="text-right">
		<br/>
		<input type="submit" name="submit" value="Сохранить" /> или <?php echo anchor(SITE_AREA .'/reports/keys', lang('keys_cancel')); ?>
	</div>
	<?php echo form_close(); ?>

	<div class="box delete rounded">
		<a class="button" id="delete-me" href="<?php echo site_url(SITE_AREA .'/content/keys/delete/'. $id); ?>" onclick="return confirm('<?php echo lang('keys_delete_confirm'); ?>')"><?php echo lang('keys_delete_record'); ?></a>
		
		<h3><?php echo lang('keys_delete_record'); ?></h3>
